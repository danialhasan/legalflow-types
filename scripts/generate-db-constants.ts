 ...
 * 
 * This script analyzes the Database type definition in database.ts
...
function generateDbConstants(): void {
  try {
    // Paths
    const typesSourcePath = path.resolve(__dirname, '../src/generated/database.ts');
    const outputPath = path.resolve(__dirname, '../src/generated/constants.ts');
    ...
    if (!sourceFile) {
      console.error('Could not process database.ts file');
      return;
    }
    ...
    console.log(`✅ Generated constants.ts with ${schemaCount} schemas and ${tableCount} tables`);
  } catch (error) {
